...
@section('admin-content')
    {!! breadcrumbs([
        ...
        'Lexicon Categories' => 'admin/data/language/lexicon-categories',
        ($category->id ? 'Edit' : 'Create') . ' Lexicon Category' => $category->id
    ...
    <p>
        Here you can specify different conjugation/declension settings for the different parts of speech set in <a
            href="{{ url('admin/data/language/lexicon-settings') }}">lexicon settings</a>. These settings are twofold;
        first, you can specify different properties, such as case or number, and the dimensions thereof. Second, once
        properties have been added, you can specify automatic conjugation/declension rules for each combination of these
        dimensions and/or for each non-dimensional property that can be applied to words within this category to
        automatically generate each form of the word. Once the category has been created, if it is a sub-category, you may
        also populate its data with that of its immediate parent. <strong>These settings are entirely optional</strong>.
    </p>
    ...
    @foreach ($classes as $class)
        <div class="mb-3">
            <h4>
                {{ $class->name }}
                <a class="small collapse-toggle collapsed" href="#{{ strtolower($class->name) }}"
            ...
            <div class="collapse" id="{{ strtolower($class->name) }}">
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <div class="d-flex">
                                                {!! Form::text(
                                                    'property_dimensions[]',
                                                    isset($values['dimensions']) ? implode(',', $values['dimensions']) : null,
                                                    ['class' => 'form-control dimension-list', 'multiple'],
                                                ) !!}
                ...
                <div class="text-right mb-3">
                    <a href="#" class="btn btn-sm btn-outline-info add-property" value="{{ $class->id }}">Add
                        Property</a>
                </div>

                @if (isset($category->data[$class->id]['properties']))
                    <h5>Auto-Conjugation/Declension Rules</h5>
                    <p>Each combination of the above dimensions and properties specified above is listed here. To set rules
                        for autogeneration of their conjugated/declined forms, enter in one or more criteria (regex
                        pattern), a regex pattern for what part of the word to replace, and what to replace it with. If
                        multiple criteria are entered, multiple replacements should be entered. Regex patterns may either be
                        entered for each criteria/replacement pair, or only one pattern may be entered for all pairs. As
                        commas may be used within regex, semicolons are used as the delimiter for these fields instead.</p>
                    <div class="row mb-2">
                        @foreach ($category->classCombinations($class->id) as $key => $combination)
                            <div
                                class="col-md-{{ count($category->data[$class->id]['properties']) <= 2 && (count(collect($category->data[$class->id]['properties'])->first()['dimensions']) == 2 || count(collect($category->data[$class->id]['properties'])->last()['dimensions']) == 2) && count($category->classCombinations($class->id)) < 20 ? 6 : (count($category->classCombinations($class->id)) % 3 == 0 && count($category->classCombinations($class->id)) < 30 ? 4 : (count($category->classCombinations($class->id)) % 4 == 0 ? 3 : (count($category->classCombinations($class->id)) < 20 ? 6 : 2))) }} mb-2">
                                    ...
                                    <div class="card-body">
                                        ...
                                        <div class="row">
                                            <div class="col-md">
                                                ...
                                                {!! Form::text(
                                                    'declension_criteria[' . $class->id . '][' . $key . ']',
                                                    isset($category->data[$class->id]['conjugation'][$key])
                                                        ? implode(';', $category->data[$class->id]['conjugation'][$key]['criteria'])
                                                        : null,
                                            ...
                                            <div class="col-md">
                                                ...
                                                {!! Form::text(
                                                    ...
                                                    isset($category->data[$class->id]['conjugation'][$key])
                                                        ? implode(';', $category->data[$class->id]['conjugation'][$key]['regex'])
                                                        : null,
                                                    ['class' => 'form-control dimension-regex'],
                                                ) !!}
                                            </div>
                                            <div class="col-md">
                                                ...
                                                {!! Form::text(
                                                    ...
                                                    isset($category->data[$class->id]['conjugation'][$key])
                                                        ? implode(';', $category->data[$class->id]['conjugation'][$key]['replacement'])
                                                        : null,
                                                    ['class' => 'form-control dimension-regex'],
                                                ) !!}
                                            </div>
    ...
    @if ($category->parent_id && isset($category->parent->data))
        <div class="form-group">
            ...
            {!! Form::label('populate_settings', 'Populate Settings', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
                'If this is turned on, this category\'s settings will be overwritten with those of its immediate parent. <strong>This overwrites any existing settings!</strong>',
            ) !!}
        </div>
    @endif
                    ...
                    <div class="form-group">
                        ...
                        <div class="d-flex">
                            {!! Form::text('property_dimensions[]', null, [
                                ...
                                'multiple',
                                'placeholder' => 'Enter any number of dimensions here, or leave blank to mark as non-dimensional',
                            ]) !!}
                            <a href="#" class="remove-property btn btn-danger ml-2 mb-2">×</a>
                        </div>
                    </div>
    ...
    <script>
        $(document).ready(function() {
            $('.delete-category-button').on('click', function(e) {
                e.preventDefault();
                loadModal(
                    "{{ url('admin/data/language/lexicon-categories/delete') }}/{{ $category->id }}",
                    'Delete Category');
            });
