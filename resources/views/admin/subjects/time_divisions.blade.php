...
@section('admin-content')
    {!! breadcrumbs([
        ...
        'Divisions' => 'admin/data/time/divisions',
    ]) !!}
    ...
    <p>
        These are the divisions of time that will be used for your site and when creating and editing events. Feel free to
        be as narrow or as broad in focus as suits your project. Setting these is semi-optional; if you do not specify any
        divisions of time, you will be able to specify only the year for any events. Once created, divisions can be sorted;
        they should be sorted from largest to smallest. If divisions are set, they will be displayed along with other basic
        information on your project's timeframe and events. It's recommended to have the largest division of time correspond
        to a year at most. <strong>Note that removing existing division(s) used for dates will impact any dates using them,
            even if the division(s) are recreated.</strong>
    </p>
    ...
    <ul>
        ...
        <li>Use for Dates, which controls whether or not the division is used when entering dates; only available for time
        ...
        @if (count($divisions))
            @foreach ($divisions as $division)
                <div class="input-group sort-item mb-3" data-id="{{ $division->id }}">
                    ...
                    {!! Form::text('abbreviation[]', $division->abbreviation, [
                        'class' => 'form-control',
                        'aria-label' => 'Division Abbreviation',
                        'placeholder' => 'Abbreviation',
                    ]) !!}
                    {!! Form::number('unit[]', $division->unit, [
                        'class' => 'form-control',
                        'aria-label' => 'Division Unit',
                        'placeholder' => 'Unit',
                    ]) !!}
                    <div class="input-group-append">
                        <div class="input-group-text">
                            {!! Form::checkbox('use_for_dates[' . $division->id . ']', 1, $division->use_for_dates, [
                                'aria-label' => 'Use for Dates',
                                'data-toggle' => 'tooltip',
                                'title' => 'Use for Dates',
                            ]) !!}
                        </div>
                        <button href="#" class="btn remove-division btn btn-danger" type="button"
