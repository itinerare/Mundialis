                        ...
                        @if (isset($page->category->template['fields'][$sectionKey]))
                            ...
                            @foreach ($page->category->template['fields'][$sectionKey] as $fieldKey => $field)
                                @switch($field['is_subsection'])
                                    @case(1)
                                        &nbsp;&nbsp;&nbsp;&nbsp;
                                        {{ $sectionLoop->iteration . '.' . $loop->iteration . '.' }} <a
                                            href="#subsection-{{ $fieldKey }}">{{ $field['label'] }}</a><br />
                                    ...
                                    @case(2)
                                        ...
                                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                        {{ $sectionLoop->iteration . '.' . (isset($fieldLoop) ? $fieldLoop->iteration : 0) . '.' . $iteration . '.' }}
                                        <a href="#subsection-{{ $fieldKey }}">{{ $field['label'] }}</a><br />
                                    @break
                                @endswitch
                            @endforeach
                            @php $iteration = 0; @endphp
