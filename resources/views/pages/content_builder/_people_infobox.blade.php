...
@foreach (['birth', 'death'] as $segment)
    @if (isset($page->data[$segment]))
        <div class="row mb-2">
            <div class="col-sm-5 bg-dark text-light rounded pt-1">
                <h6><strong>{{ ucfirst($segment) }}</strong></h6>
            </div>
            <div class="col-sm">
                <div class="pt-1">
                    {!! isset($page->data[$segment]['date']) ? $dateHelper->formatTimeDate($page->data[$segment]['date']) : '' !!}{!! isset($page->data[$segment]['chronology'])
                        ? ' ' . App\Models\Subject\TimeChronology::where('id', $page->data[$segment]['chronology'])->first()->displayName
