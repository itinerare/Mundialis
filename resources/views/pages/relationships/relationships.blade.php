...
@section('pages-content')
    ...
    <div class="text-right mb-4">
        @if ($page->personRelations())
            <a href="{{ url('pages/' . $page->id . '/relationships/tree') }}" class="btn btn-secondary mt-4 ml-2">Family
                Tree</a>
        @endif
            ...
            <div class="row mb-4">
                @foreach ([$relationship->page_one_id == $page->id ? 'one' : 'two', $relationship->page_one_id == $page->id ? 'two' : 'one'] as $iteration)
                    @include('pages.relationships._relationship', [
                        ...
                        'relationshipPage' =>
                            $iteration == 'one' ? $relationship->pageOne : $relationship->pageTwo,
                        'type' => $iteration == 'one' ? $relationship->type_one : $relationship->type_two,
                        'displayType' =>
                            $iteration == 'one' ? $relationship->displayTypeOne : $relationship->displayTypeTwo,
                    ])
                @endforeach
                @if (Auth::check() && Auth::user()->canEdit($page))
                    <div class="col-sm-1 text-center">
                        <div class="row h-100">
                            <div class="col mb-2">
                                <a href="#" data-id="{{ $relationship->id }}"
                                    class="edit-relationship-button btn btn-primary h-100 w-100">
                            ...
                            <div class="col mb-2">
                                <a href="#" data-id="{{ $relationship->id }}"
                                    class="delete-relationship-button btn btn-danger h-100 w-100">
                                    <div class="mobile-hide mt-3">
    ...
    <div class="text-center mt-4 small text-muted">{{ $relationships->total() }}
        result{{ $relationships->total() == 1 ? '' : 's' }} found.</div>
    ...
    <script>
        $(document).ready(function() {
            $('.add-relationship-button').on('click', function(e) {
                e.preventDefault();
                loadModal("{{ url('pages') }}/{{ $page->id }}/relationships/create",
                    'Create Relationship');
            });

            $('.edit-relationship-button').on('click', function(e) {
                e.preventDefault();
                loadModal("{{ url('pages') }}/{{ $page->id }}/relationships/edit/" + $(this).data(
                    'id'), 'Edit Relationship');
            });

            $('.delete-relationship-button').on('click', function(e) {
                e.preventDefault();
                loadModal("{{ url('pages') }}/{{ $page->id }}/relationships/delete/" + $(this).data(
