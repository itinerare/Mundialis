<div class="col-md text-center d-flex flex-column">
    @if ($member['page']->personRelations('parents'))
        ...
        @if ($member['page']->personRelations('parents')->count() > 1)
            <div class="border-top-0 border border-secondary rounded-bottom mb-2"
                style="width:{{ 25 * $member['page']->personRelations('parents')->count() }}%; margin-left:{{ (100 - 25 * $member['page']->personRelations('parents')->count()) / 2 }}%; height:20px;">
            </div>
        @endif
    @endif
    <div class="mt-auto align-bottom">
        @if ($member['page']->image)
            <img src="{{ Storage::url($member['page']->image->thumbnailUrl) }}" style="width:100px;" class="img-thumbnail mw-100" />
        @else
