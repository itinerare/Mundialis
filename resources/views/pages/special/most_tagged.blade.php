...
@section('pages-content')
    ...
    <ul>
        @foreach ($pages as $taggedPage)
            <li>
                {!! $taggedPage->displayName !!} ({{ $taggedPage->tags->count() }}
                tag{{ $taggedPage->tags->count() != 1 ? 's' : '' }}) <a class="collapse-toggle collapsed"
                    href="#group-{{ $taggedPage->id }}" data-toggle="collapse">Show <i class="fas fa-caret-right"></i></a>
                </h3>
