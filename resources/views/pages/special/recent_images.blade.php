...
@section('pages-title') Special - Recent Image Changes @endsection
...
<p class="text-right">
    ...
    @foreach([
        ...
        30 => '30 Days', 50 => '50 Days', 'all' => 'All Time'
    ] as $mode=>$label)
        @if(Request::url().(Request::get('mode') ? '?mode='.Request::get('mode') : '' ) == url('special/recent-images?mode='.$mode))
            {{ $label }}
        @else
