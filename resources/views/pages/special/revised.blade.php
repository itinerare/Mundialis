...
@section('pages-title') Special - {{ ucfirst($mode) }} Revised Pages @endsection
...
<ul>
    @foreach($pages as $versionPage)
        <li>
            {!! $versionPage->displayName !!} ({{ $versionPage->versions->count() }} revision{{ $versionPage->versions->count() != 1 ? 's' : '' }})
        </li>
    @endforeach
</ul>
